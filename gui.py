...
def beam_search(model, tokenizer, input_sentence, beam_width=3, max_decoded_sentence_length=20):
  """Performs beam search decoding for French translation.

  Args:
      model: The loaded French translation model.
      tokenizer: The tokenizer for English text.
      input_sentence: The English sentence to translate.
      beam_width: The beam width for beam search (number of candidates to consider).
      max_decoded_sentence_length: The maximum length of the decoded French sentence.

  Returns:
      The best translated French sentence.
  """
  ...
  input_sequence = pad(input_sequence, max_length)
  ...
  for i in range(max_decoded_sentence_length):
    ...
    expanded_candidates = []
    for prob, partial_sequence in candidates:
      # Predict the next word probabilities
      prediction = model.predict([input_sequence, np.array([partial_sequence])])[0]
      
      # Get the top `beam_width` most probable words
      top_beam_words = np.argsort(prediction[0])[-beam_width:]

      # Generate new candidate sequences
      for word_index in top_beam_words:
        new_prob = prob + prediction[0][word_index]
        new_sequence = partial_sequence + [word_index]
        expanded_candidates.append((new_prob, new_sequence))

    # Sort the expanded candidates by probability
    sorted_candidates = sorted(expanded_candidates, key=lambda x: x[0], reverse=True)

    # Select the top `beam_width` candidates for the next iteration
    candidates = sorted_candidates[:beam_width]
    ...
    for prob, sequence in candidates:
      if sequence[-1] == end_token:
        ...
        translated_sentence = tokenizer.sequences_to_texts([sequence])[0]
        return translated_sentence.replace("[start]", "").replace("[end]", "")

  # No candidate reached the end token within the maximum length
  return None
...
def handle_translate():
    ...
    try:
        if selected_language == "French":
            translation= beam_search(model, english_tokenizer, english_sentence)
...
def start_recording():
    if not is_after_6pm():
        messagebox.showerror("Please try after \n 6 PM IST.")
        return
